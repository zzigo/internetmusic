...
let s = function(p) {
  ...
  let myCapture; // camera (or any available capture device)
  let myVida;    // VIDA
  ...
  */
  p.initCaptureDevice = function() {
    try {
      ...
      myCapture.hide();
      let tempInfoString = '[initCaptureDevice] capture ready. Resolution: ' +
      ...
      let tempInfoString = '[initCaptureDevice] capture error: ' + _err;
      if((maxIsDetected)) {
        window.max.outlet('console', tempInfoString);
      }
      else {
    ...
    if(maxIsDetected) {
      ...
      */
      window.max.bindInlet('remove_all_zones', function () {
        ...
        */
        for(let i = 0; i < myVida.activeZones.length; i++) {
          if(
      ...
      let temp_n = 5;
      let temp_padding = 1.0 / temp_n;
      let temp_w = temp_padding * 0.6;
      let temp_offset_x = temp_padding * 0.4 * 0.5;
      for(let i = 0; i < temp_n; i++) {
