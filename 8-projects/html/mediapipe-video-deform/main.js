class VideoDeformer {
  ...
  async init() {
    try {
      console.log("Initializing video and hand tracking...");
      ...
      errorMessage.textContent = `Error: ${error.message}. Please allow camera access and ensure you're using a modern browser.`;
  ...
  onResults(results) {
    ...
    if (results.multiHandLandmarks) {
      ...
      for (const landmarks of results.multiHandLandmarks) {
        // Draw index fingertip with a circle
        const fingerTip = landmarks[8];
        this.ctx.beginPath();
        this.ctx.arc(
          fingerTip.x * this.canvas.width,
          fingerTip.y * this.canvas.height,
          5,
          0,
          2 * Math.PI
        );
